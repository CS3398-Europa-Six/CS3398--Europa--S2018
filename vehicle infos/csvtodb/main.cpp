...
const char * tablename = "cqa";
const char * infilename = "cqa.csv";
const char * outfilename = "cqa.sql";
    ...
    while (std::getline(infile,buffa)) {
        ...
        for (int i= 0;i<buffa.length();++i) {
            ...
            if (buffa[i] == ',') {
                outbuffa += "\'";
                ++currentelement;
nulltst:        if (isexcluded(currentelement)) {
                    ++currentelement;
                    for (++i;i<buffa.length();++i) {
                        if (buffa[i] == ',') break;
                    }
                ...
                if (i<=buffa.length()-5) {
                    if (buffa[i+1] == 'N' && buffa[i+2] == 'U' &&
                        buffa[i+3] == 'L' && buffa[i+4] == 'L') {
                        i = i + 5;
                        ++currentelement;
                        outbuffa += "\',\'";
                        goto nulltst;
